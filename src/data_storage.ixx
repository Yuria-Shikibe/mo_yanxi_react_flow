...
namespace mo_yanxi::react_flow{
	...
	public:
		...
		[[nodiscard]] constexpr explicit data_package(const T& to_ref) noexcept : is_local(false),
			u{std::addressof(to_ref)}{
		}
	...
	public:
		...
		[[nodiscard]] T fetch_unchecked() const noexcept(std::is_nothrow_copy_constructible_v<T> &&
			std::is_nothrow_move_constructible_v<T>){
			if constexpr(is_small_object){
	...
	public:
		...
		template <typename T, typename... Args>
			requires (sizeof(T) <= local_storage_size && alignof(T) <= local_storage_align && std::constructible_from<T,
				Args&&...>)
		[[nodiscard]] data_storage(std::in_place_type_t<T>,
