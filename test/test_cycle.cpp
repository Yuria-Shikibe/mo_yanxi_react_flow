...
TEST(CycleTest, SimpleCycle) {
    manager mgr;
    // t1: int -> int (identity)
    auto& t1 = mgr.add_node(make_transformer(propagate_behavior::eager, async_type::none, [](int v){ return v; }));

    // t2: int -> int (identity)
    auto& t2 = mgr.add_node(make_transformer(propagate_behavior::eager, async_type::none, [](int v){ return v; }));
...
TEST(CycleTest, LargerCycle) {
    ...
    auto& t1 = mgr.add_node(make_transformer(propagate_behavior::eager, async_type::none, [](int v){ return v; }));
    auto& t2 = mgr.add_node(make_transformer(propagate_behavior::eager, async_type::none, [](int v){ return v; }));
    auto& t3 = mgr.add_node(make_transformer(propagate_behavior::eager, async_type::none, [](int v){ return v; }));
